...
if(verbose>1) then 
   ...
   !print '(a, i8, a, f5.1, "GB")',"  >>>>>>>>>>>>    N_TOTAL: ",(n+1)*(m+l+1) , ' ; Approximate memory needed: ', (n+1)*(m+l+1)*(min(n,m+l)+5)*8.e-9
...
write(4000+k,50) r(i),u(js,i),Er,Ez,-Ezbelow , -eps_r*Ezbelow ! two last should be the same! check the output
enddo
close(4000+k)
end subroutine WriteOutputs

!===============================================
subroutine WriteParameters(aa,bb)
real*8 :: aa(-l:m,0:n), bb(-l:m,0:n)
character*200 str
write(str,'(a,f6.4)')"Parameters.dat",Z/Rtip
open(601,file=str)
write(601,*) "# rho(nm)  z(nm)  hn(nm)  hm(nm)  aa  bb"
do i=0,n
  do j=-l,m
     write(601,'(6e18.8)') r(i), zm(j), hn(i), hm(j), aa(j,i), bb(j,i)
  enddo
enddo
close(601)
...
!===============================================
subroutine NodalFields
real*8 Er,Ez,E2
character*200 str
write(str,'(a,f6.4)')"Fields.dat",Z/Rtip
open(k+1000,file=str)
write (k+1000,*), "# rho(nm)  z(nm)  pot(V)  E2(1/nm)2  E_rho(1/nm)  E_z(1/nm)"
do i=0,n
   do j=-l,m
      if(i==0) then 
            Er =-(u(j,i+1)-u(j,i))/hn(i)
      ...
      E2=Ez**2+Er**2
     write(k+1000,'(2e18.8,e28.18,3e18.8)')r(i),zm(j),u(j,i) , E2 , Er, Ez
  enddo
  write(k+1000,*)
enddo
close(k+1000)
end subroutine NodalFields
...
do i=0,max(nApex,nCone,nLever,nEdge)
  ...
  j=Probebot(i) -1 ! avoid stepwise boundary 
  Er=-(u(j,i+1)-u(j,i))/hn(i)
  Ez=-(u(j,i)-u(j-1,i))/hm(j)
  E2=Er**2+Ez**2
  df=.5_dp*(r(i)+r(i+1)) * hn(i) * E2
  Force=Force+df
  write(500,'(18es18.8)'),.5_dp*(r(i)+r(i+1)),Force,dF,zm(Probebot(i)),E2,er,ez,u(j,i),u(j,i+1),u(j-1,i)
...
!  constructing constant coefficients  aa ,bb. => they depend only  on geometry and Z
do i=0,n-1
do j=js+1,m
...
write(* ,'(a,f9.4,e13.3 )')    ' ===S/R,C/pi.e0.R:',Z/Rtip,2_dp*energy/Rtip 
write(* ,'(60x,a,f5.2,e13.3,f10.2 )')    ' +++Separation,Capacitance, cpu time: ',Z,2_dp*energy*pi_e0 ,  t
!write(200 ,'(f10.5,2es )')     Z,energy*2*pi_e0  ! C=2U/V2 and V=1
write(201 ,'(f10.5,2es18.8 )')     Z/Rtip,energy/Rtip,  2._dp*energy/Rtip 
