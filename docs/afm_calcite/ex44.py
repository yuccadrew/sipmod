"""
Solve 3D Poisson Nernst Planck equations for AFM calcite model with 98 nm
tip-sample separation. Use capsol.post_processing to process solution. Fix
concentrations and sigma at the solid-water interface.
"""
...
from docs.afm_calcite.ex43 import capsol, mesh_prefix  # noqa
...
def run():
    ...
    prev_mesh = MeshTri.read('docs/afm_calcite/meshes/mesh_ex41.1',
                             axis_of_symmetry='Y',
                             scale_factor=1e-9)
    dof = 4  # must be 4 in this case
    indices = [mesh.subdomain_nodes('water')['id'] * dof + i
               for i in range(phys.nions)]
    prev_indices = [prev_mesh.subdomain_nodes('water')['id'] * dof + i
                    for i in range(phys.nions)]
    indices += [mesh.boundary_nodes('stern')['id'] * dof + i
                for i in range(dof-1, dof)]
    prev_indices += [prev_mesh.boundary_nodes('stern')['id'] * dof + i
                     for i in range(dof-1, dof)]
    indices = np.hstack(indices)
    prev_indices = np.hstack(prev_indices)
    prev_dcEM = load_dict('docs/afm_calcite/meshes/ex42t0.h5', 'dcEM')
    prev_fEM = load_dict('docs/afm_calcite/meshes/ex42t0.h5', 'fEM')
    ...
    amend_dict(path+'/ex44.h5', data={'dcEM': x,
                                      'dcEr': Er,
                                      'dcEz': Ez,
                                      'dcFz': Fz})
    ...
    amend_dict(path+'/ex44.h5', data={'fEM': x,
                                      'fEr': Er,
                                      'fEz': Ez,
                                      'fFz': Fz})
...
def validate():
    ...
    prev_mesh = MeshTri.read('docs/afm_calcite/meshes/mesh_ex41.1',
                             axis_of_symmetry='Y',
                             scale_factor=1e-9)
    indices = [mesh.subdomain_nodes('water')['id'] * 4 + i
               for i in range(phys.nions)]
    prev_indices = [prev_mesh.subdomain_nodes('water')['id'] * 4 + i
                    for i in range(phys.nions)]
    indices += [mesh.boundary_nodes('stern')['id'] * 4 + i
                for i in range(3, 4)]
    prev_indices += [prev_mesh.boundary_nodes('stern')['id'] * 4 + i
                     for i in range(3, 4)]
    indices = np.hstack(indices)
    prev_indices = np.hstack(prev_indices)
    print('prev_indices', prev_indices.shape[0])
    print('indices', indices.shape[0])

    path = os.path.dirname(mesh_prefix)
    # prev_u = load_dict('docs/afm_calcite/meshes/ex42t0.h5', 'dcEM')
    prev_u = load_dict('docs/afm_calcite/meshes/ex42t0.h5', 'fEM')[-1, :]
    # u = load_dict(path+'/ex44.h5', 'dcEM')
    u = load_dict(path+'/ex44.h5', 'fEM')[-1, :]
...
def visualize():
    import matplotlib.pyplot as plt
    from sipmod.utils import load_dict
    prev_dcFz = load_dict('docs/afm_calcite/meshes/ex42t0.h5', 'dcFz')
    prev_fFz = load_dict('docs/afm_calcite/meshes/ex42t0.h5', 'fFz')

    path = os.path.dirname(mesh_prefix)
    dcEM = load_dict(path+'/ex44.h5', 'dcEM')
    dcFz = load_dict(path+'/ex44.h5', 'dcFz')
    fEM = load_dict(path+'/ex44.h5', 'fEM')
    fFz = load_dict(path+'/ex44.h5', 'fFz')
